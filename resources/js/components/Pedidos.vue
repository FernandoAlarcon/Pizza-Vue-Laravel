                        ...
                        <div class="row" >
                            <div class="col-md-12" >
                                ...
                                <div>
                                    <label>(Cliente)</label>
                                    <select class="form-control" 
                                            v-model="PedidosListo.Usuario" 
                                            name=""
                                              > 
                                        <option  
                                            v-for="User of ListUsers" 
                                            :value="User.id" >
                                            {{User.name}} 
                                        </option>
                                    </select><br>
                            ...
                            <table class="table table-hover table-striped " >
                                ...
                                <tbody>
                                    <tr v-for="pedido of DataPedidos" >
                                        ...
                                        <td width="10px"  v-if="pedido.Estado_Pedido === 'Preparacion'"  >
                                            <button type="button" 
                                                    v-if="pedido.Estado_Pedido === 'Preparacion'" 
                                                    class="btn btn-warning"
                                                    v-on:click.prevent="EstadoPedidos(pedido.id_pedido,'Cancelado',pedido.email,pedido.name)" name="button">
                                                Cancelar 
                                            </button>
                                        </td>
                                        <td  v-if="pedido.Estado_Pedido === 'Preparacion'"  >
                                            <button type="button" 
                                                    v-if="pedido.Estado_Pedido === 'Preparacion'"
                                                    class="btn btn-success"
                                                    v-on:click.prevent="EstadoPedidos(pedido.id_pedido,'Realizado',pedido.email,pedido.name)" name="button">
                                                Realizado
                                            </button>
                                        </td>
                                         <td colspan="2" v-if="pedido.Estado_Pedido != 'Preparacion'"  >
                                            <center>
                                                <button type="button" 
                                                        v-if="pedido.Estado_Pedido === 'Cancelado'"
                                                        class="btn btn-danger"
                                                        v-on:click.prevent="EstadoPedidos(pedido.id_pedido,'Eliminado',pedido.email,pedido.name)" name="button">
...
//@change="CalcularPrecio"
    export default {
        ...
        methods:{ 
            ...
            EstadoPedidos(id_pedido, estado, correo, nombre){ 
            ...
            VerPedidos(){
                axios.get('/PedidosAdmin',{ params:{PedidoA:this.busqueda_Pedidos}}).then(response => {
            ...
            AgregarPedido(){ 
                ...
                axios.post(url,{
                        ...
                        Usuario     : this.PedidosListo.Usuario
                }).then(response=>{ 
                    let Event = response.data.Informacion.Event
                    if(Event == 'successfull'){
